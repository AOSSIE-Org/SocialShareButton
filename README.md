...
- ⚛️ Framework support: React, Next.js, Vue, Angular, Solid.js, or plain HTML
...
### Solid.js Wrapper Component (Optional)

Copy `src/social-share-button-solid.jsx` to your SolidStart or Solid.js project. The wrapper uses `onMount` / `onCleanup` for lifecycle management and `createEffect` to reactively sync props:

```jsx
// App.jsx (SolidStart page or any Solid.js component)
...
> **SolidStart / SSR note:** All browser-API calls are guarded with
> `typeof window !== 'undefined'` inside `onMount`, so the component is safe
> to use in server-rendered Solid applications.
