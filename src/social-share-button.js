 ...
 *   og:title → twitter:title → semantic h1 (article/main/landmark) → document.title
 *
 * Detection priority for description / excerpt:
 *   og:description → twitter:description → meta[name="description"] → body text excerpt
 *
 * Results are cached for 30 s per instance so repeated calls within the same
...
const _ContentDetector = (() => {
  ...
  function _detectTitle(doc) {
    ...
    const tw =
      _getMeta(doc, 'meta[name="twitter:title"]') ||
      _getMeta(doc, 'meta[property="twitter:title"]');
    if (tw) return tw;
    ...
    // Strip trailing " | SiteName" suffix from document.title
  ...
  function _detectMetaDesc(doc) {
    return (
      ...
      _getMeta(doc, 'meta[name="twitter:description"]') ||
      _getMeta(doc, 'meta[property="twitter:description"]') ||
      _getMeta(doc, 'meta[name="description"]')
    );
  }
  ...
  function _excerpt(text, minLen = 140, maxLen = 200) {
    ...
    const wordEnd = win.lastIndexOf(' ');
    return wordEnd > minLen ? win.slice(0, wordEnd).trim() + '\u2026' : win.trim() + '\u2026';
  }

  return {
    /**
     * Extract sharing metadata from the document.
     * Returns { title, excerpt, textContent }.
     * Never throws — falls back gracefully on any DOM error.
     *
     * @param {Document} doc
    ...
    extract(doc, bustCache = false) {
      ...
      let result;
      try {
        const title = _detectTitle(doc);
        let excerpt = _detectMetaDesc(doc);
        let textContent = '';
        if (!excerpt) {
          const root = _findContentRoot(doc);
          textContent = _toPlainText(root);
          excerpt = _excerpt(textContent);
        } else {
          try {
            textContent = _toPlainText(_findContentRoot(doc));
          } catch (_) {
            textContent = excerpt;
          }
        }
...
class SocialShareButton {
  constructor(options = {}) {
    // -------------------------------------------------------------------------
    // Intelligent content auto-detection (Issue #26)
    //
    // When title or description are not provided by the caller, attempt to
    // detect them from the page's metadata and semantic HTML.  Manual props
    // always win — auto-detection only fills in what's missing.
    //
    // Set autoDetect: false to disable entirely.
    // -------------------------------------------------------------------------
    let _autoTitle = '';
    let _autoDescription = '';
    ...
    this.options = {
      ...
      // Content auto-detection — set to false to disable (e.g. when all props are always provided)
...
/**
 * Clears the internal content-detection cache.
 *
 * Call this after a client-side navigation (SPA route change) so the next
 * SocialShareButton instance picks up freshly detected content.
 *
 * @example
