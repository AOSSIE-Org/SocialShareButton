...
class SocialShareButton {
  constructor(options = {}) {
    // Initialize configuration options with provided values or defaults
    this.options = {
    ...
    // If a container is provided, initialize the component immediately
  ...
  init() {
    // Create the main trigger button if it is enabled in options
    if (this.options.showButton) {
      this.createButton();
    }
    // Setup the modal dialog overlay
    this.createModal();
    // Bind all necessary user interaction events
    this.attachEvents();
    // Apply custom styling colors to the button if configured
  ...
  createButton() {
    ...
    this.button = button;
    // Append the button to the specified container if one exists
    if (this.options.container) {
      const container =
  ...
  createModal() {
    // Create the background overlay element for the modal
    ...
    // Find the container for the link input and create the input element
    ...
    // Store a reference to the modal and append it directly to the document body
    this.modal = modal;
    document.body.appendChild(modal);
  }

  getPlatformsHTML() {
    // Define platform configuration including names, colors, and SVG icons
    const platforms = {
      whatsapp: {
        name: 'WhatsApp',
        color: '#25D366',
    ...
    // Filter out unsupported platforms and generate HTML for each requested platform
  ...
  getShareURL(platform) {
    // Extract sharing parameters from options and encode them for use in URLs
    ...
    // Return the specific URL for the requested platform or an empty string if not found
  ...
  openModal() {
    // Abort if the modal DOM element no longer exists
    // Safety check: prevent errors if modal was destroyed
    if (!this.modal) return;

    // Update state and make the modal visible
    ...
    // Manage body scroll locking to prevent background scrolling when modal is open
    ...
    // Animate in
    // Trigger a slight delay before adding the active class for smooth CSS transitions
  ...
  closeModal() {
    // Ensure the modal exists before attempting to close it
    if (!this.modal) return; // Safety check

    // Remove the active class to start the fade-out CSS transition
    ...
    // Wait for the transition to finish before fully hiding the modal
  ...
  share(platform) {
    // Retrieve the specific sharing endpoint for the chosen platform
    const shareUrl = this.getShareURL(platform);

    // Proceed if a valid sharing URL is found
    if (shareUrl) {
  ...
  copyLink() {
    // Get references to the input field holding the URL and the copy button itself
    const input = this.modal.querySelector('.social-share-link-input input');
    const copyBtn = this.modal.querySelector('.social-share-copy-btn');
  ...
  destroy() {
    // Remove the global keydown event listener if it was attached
  ...
  applyCustomColors() {
    // Abort if the button element has not been created yet
