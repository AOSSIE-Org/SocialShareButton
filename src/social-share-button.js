...
class SocialShareButton {
  constructor(options = {}) {
    ...
    this.previousActiveElement = null; // Saved element to restore focus on modal close
    ...
    // Instance-scoped unique ID for aria-labelledby to avoid DOM ID collisions
    this._modalTitleId = `social-share-modal-title-${Date.now()}-${Math.random().toString(36).slice(2, 9)}`;
  ...
  /**
   * Creates an SVG element with a single <path> child using secure DOM APIs.
   * @param {string} viewBox - The SVG viewBox attribute value.
   * @param {string} pathData - The "d" attribute for the <path> element.
   * @param {Object} [attrs] - Optional extra attributes to set on the <svg> element.
   * @returns {SVGSVGElement}
   */
  _createSVG(viewBox, pathData, attrs = {}) {
    const NS = "http://www.w3.org/2000/svg";
    const svg = document.createElementNS(NS, "svg");
    svg.setAttribute("viewBox", viewBox);
    svg.setAttribute("xmlns", NS);
    // Apply optional attributes (e.g. width, height, fill, class)
  ...
  createButton() {
    ...
    const svg = this._createSVG(
      ...
      { width: "20", height: "20", fill: "none", class: "share-icon" }
    );
  ...
  createModal() {
    ...
    modal.setAttribute("aria-labelledby", this._modalTitleId);

    const content = document.createElement("div");
    content.className = `social-share-modal-content ${this.options.modalPosition}`;

    const header = document.createElement("div");
    header.className = "social-share-modal-header";
    
    const title = document.createElement("h3");
    title.id = this._modalTitleId;
    title.textContent = this.options.modalTitleText;
    header.appendChild(title);

    const closeBtn = document.createElement("button");
    closeBtn.type = "button";
    closeBtn.className = "social-share-modal-close";
    closeBtn.setAttribute("aria-label", this.options.closeButtonText);
    closeBtn.textContent = "✕";
    header.appendChild(closeBtn);

    const platformsContainer = document.createElement("div");
    platformsContainer.className = "social-share-platforms";
    this.buildPlatformButtons(platformsContainer);

    const linkContainer = document.createElement("div");
    linkContainer.className = "social-share-link-container";

    const urlInputContainer = document.createElement("div");
    urlInputContainer.className = "social-share-link-input";
    
    const urlInput = document.createElement("input");
    urlInput.type = "text";
    urlInput.value = this.options.url;
    urlInput.readOnly = true;
    urlInput.setAttribute("aria-label", this.options.urlInputAriaLabel);
    urlInputContainer.appendChild(urlInput);

    const copyBtn = document.createElement("button");
    copyBtn.type = "button";
    copyBtn.className = "social-share-copy-btn";
    copyBtn.textContent = this.options.copyButtonText;

    linkContainer.appendChild(urlInputContainer);
  ...
  buildPlatformButtons(container) {
    const docFrag = document.createDocumentFragment();
    
    // Ignore unknown platform keys that do not exist in the PLATFORMS map
    this.options.platforms.forEach((platform) => {
      if (PLATFORMS[platform]) {
        ...
        btn.type = "button";
        btn.className = "social-share-platform-btn";
        btn.dataset.platform = platform;
        btn.style.setProperty("--platform-color", color);

        const iconDiv = document.createElement("div");
        iconDiv.className = "social-share-platform-icon";
        iconDiv.style.backgroundColor = color;
        
        const svg = this._createSVG("0 0 24 24", icon, { fill: "white" });
  ...
  attachEvents() {
    ...
    const platformsDelegationHandler = (e) => {
      const btn = e.target.closest(".social-share-platform-btn");
      if (btn) {
        const platform = btn.dataset.platform;
        this.share(platform);
      }
    };
    this.addEventListener(platformsContainer, "click", platformsDelegationHandler);
    ...
    this.handleKeydown = (e) => {
      ...
      if (e.key === "Escape") {
        if (this.closeTimeout) return; // Prevent duplicate close handling
        this.closeModal();
        return;
      }
  ...
  trapFocus(e) {
    ...
    const isOutsideModal = !this.modal.contains(document.activeElement);
  ...
  openModal() {
    ...
    // Save previous active element for restoration on close, only if modal is currently closed
    if (!this.isModalOpen) {
