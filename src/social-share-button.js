...
class SocialShareButton {
   ...
   */
  constructor(options = {}) {
    /** @type {Object} Resolved configuration, merging caller options with defaults. */
    this.options = {
    ...
    /** @type {boolean} Whether the share modal is currently visible. */
    this.isModalOpen = false;

    /** @type {HTMLElement|null} The modal overlay element appended to document.body. */
    this.modal = null;

    /** @type {HTMLElement|null} The trigger button element mounted inside the container. */
    this.button = null;

    /** @type {Function|null} Stored mouseenter handler for custom button colour; kept for clean removal. */
    this.customColorMouseEnterHandler = null;

    /** @type {Function|null} Stored mouseleave handler for custom button colour; kept for clean removal. */
    this.customColorMouseLeaveHandler = null;

    /** @type {Function|null} Document-level keydown handler for ESC-to-close; stored for removal in destroy(). */
    this.handleKeydown = null;

    /** @type {Array<{element: EventTarget, type: string, handler: Function, options: *}>}
     *  Central registry of all event listeners added via addEventListener(), used for bulk removal. */
    this.listeners = [];

    /** @type {number|null} setTimeout ID for the open-modal CSS transition (10 ms delay). */
    this.openTimeout = null;

    /** @type {number|null} setTimeout ID for the close-modal CSS transition (200 ms delay). */
    this.closeTimeout = null;

    /** @type {number|null} setTimeout ID that resets the Copy button label after 2 s. */
    this.feedbackTimeout = null;

    /** @type {boolean} True while this instance holds the body overflow lock, preventing double-locking. */
    this.ownsBodyLock = false;

    /** @type {boolean} Re-entrancy guard; set to true after attachEvents() runs to prevent duplicate listeners. */
    this.eventsAttached = false;

    /** @type {boolean} Set to true by destroy(); guards async callbacks (clipboard, timeouts) from running after teardown. */
  ...
  /**
   * Bootstraps the component: creates the button (if enabled), the modal,
   * attaches all event listeners, and applies any custom colours.
   * Called automatically when a container is passed to the constructor.
   */
  init() {
  ...
  /**
   * Creates the trigger <button> element with an SVG share icon and mounts it
   * inside the configured container (resolved from a CSS selector string or a
  ...
  /**
   * Builds the modal overlay DOM structure — including the platform grid,
   * read-only URL input, and Copy button — and appends it to document.body.
   * The input element is created programmatically to avoid XSS via innerHTML.
   */
  createModal() {
    ...
    // Append URL input separately to avoid setting user-supplied URL via innerHTML.
  ...
  /**
   * Generates the inner HTML for the platform button grid.
   * Only platforms present in both the built-in map and options.platforms are rendered.
   * Each button stores its platform key in a data-platform attribute for event delegation.
   * @returns {string} HTML string of platform <button> elements.
   */
  getPlatformsHTML() {
    /** @type {Object.<string, {name: string, color: string, icon: string}>}
     *  Built-in platform definitions: display name, brand colour, and SVG path data. */
    const platforms = {
      whatsapp: {
        name: 'WhatsApp',
        color: '#25D366',
  ...
  /**
   * Builds the platform-specific share URL for the given platform key.
   * Each platform receives a tailored message combining title, description,
   * hashtags, and encoded URL; the via parameter is appended for Twitter only.
   * @param {string} platform - One of the supported platform keys (e.g. 'twitter').
   * @returns {string} A fully-encoded share URL, or an empty string if the platform is unknown.
   */
  getShareURL(platform) {
    ...
    /** @type {string} Space-separated hashtag string prefixed with '#', or empty if none provided. */
    ...
    /** @type {Object.<string, string>} Map of platform key to its fully-encoded share URL. */
    const urls = {
  ...
  /**
   * Wraps the native addEventListener and records the listener in the central
   * registry so it can be removed en-masse by removeAllListeners().
   * @param {EventTarget}  element  - The DOM node or object to listen on.
   * @param {string}       type     - The event type string (e.g. 'click').
   * @param {Function}     handler  - The event handler callback.
   * @param {boolean|Object} [options=false] - addEventListener options / useCapture flag.
   */
  ...
  /**
   * Removes all event listeners that were registered through addEventListener()
   * and clears the registry. Called by destroy() to prevent memory leaks.
   */
  removeAllListeners() {
  ...
  /**
   * Registers all interactive event listeners for the button, modal overlay,
   * close button, platform buttons, copy button, URL input, and ESC key.
   * Protected by eventsAttached to ensure it only runs once per instance.
   */
  attachEvents() {
    ...
    // Modal overlay click to close (only when clicking the backdrop, not the content)
    ...
    // Platform buttons — each reads its platform key from data-platform
    ...
    // Copy button triggers clipboard write or execCommand fallback
    ...
    // Clicking the URL input auto-selects all text for easy manual copying
    ...
    // ESC key closes the modal; stored on the instance for explicit removal in destroy()
  ...
  /**
   * Makes the modal visible and locks page scrolling.
   * Uses a shared static counter (openModalCount) so multiple simultaneous
   * instances do not fight over document.body.style.overflow.
   * A 10 ms setTimeout triggers the CSS enter-transition after the display change.
   */
  openModal() {
    ...
    // Animate in: add 'active' class after a micro-delay so the CSS transition fires
  ...
  /**
   * Hides the modal after a 200 ms CSS exit-transition and restores page
   * scrolling once all open instances have closed.
   * Race conditions with openModal() are resolved by cancelling pending timeouts.
   */
  closeModal() {
    ...
    // Wait for the CSS transition to finish before hiding and unlocking scroll
  ...
  /**
   * Opens the share URL for the given platform in a new browser tab,
   * or navigates the current tab for email (mailto: links).
   * Fires the onShare callback after the window is opened.
   * @param {string} platform - The platform key to share to (e.g. 'twitter').
   */
  share(platform) {
    ...
    if (shareUrl) {
      if (platform === 'email') {
        // mailto: links must use location.href — window.open is blocked for mailto in some browsers
  ...
  /**
   * Copies the share URL to the clipboard using the async Clipboard API when
   * available, falling back to the legacy execCommand('copy') path for older
   * browsers. Updates the Copy button label to 'Copied!' for 2 s as feedback.
   */
  copyLink() {
    const input = this.modal.querySelector('.social-share-link-input input');
    const copyBtn = this.modal.querySelector('.social-share-copy-btn');
    ...
    if (navigator.clipboard && navigator.clipboard.writeText) {
      navigator.clipboard
        ...
        .catch(() => {
          // Clipboard API rejected (e.g. permissions denied) — fall back to execCommand
   ...
   * Selects the full input value (including on mobile via setSelectionRange),
   * executes the copy command, and mirrors the same button feedback as copyLink().
   * Shows 'Failed' for 2 s if execCommand throws.
   * @param {HTMLInputElement} input   - The read-only URL input element.
   * @param {HTMLButtonElement} copyBtn - The Copy button whose label is updated.
   */
  fallbackCopy(input, copyBtn) {
    ...
    try {
      ...
      // execCommand failed — show brief error feedback without exposing internals
  ...
  /**
   * Fully tears down the instance: removes all event listeners, cancels pending
   * timeouts, detaches the ESC key handler, removes the button and modal from the
   * DOM, releases the body scroll lock if held, and nulls all references so the
   * instance can be safely garbage-collected. Idempotent — safe to call multiple times.
   */
  destroy() {
    // Remove the document-level ESC key listener first (registered outside the central registry)
    ...
    // Remove custom color handlers (managed outside the central registry)
  ...
  /**
   * Merges new option values into the current configuration at runtime.
   * Updates the URL input inside the modal immediately if it exists,
   * and re-applies custom button colours when colour options change.
   * @param {Object} options - Partial options object; only provided keys are updated.
   */
  ...
  /**
   * Applies buttonColor and buttonHoverColor to the trigger button via inline styles.
   * Removes any legacy global <style> tag that older versions injected to avoid
   * colour bleeding between multiple instances on the same page.
   * Hover behaviour is handled with mouseenter/mouseleave rather than CSS :hover so
   * the exact colour values from options are always respected.
   */
  applyCustomColors() {
    ...
    // Remove previous handlers before re-applying to avoid stacking duplicates
    ...
    // Reset inline styles so a clean base colour can be applied
    ...
    /** @type {string} Base button colour; empty string means use CSS default. */
    const baseColor = this.options.buttonColor || '';

    /** @type {string} Hover colour; falls back to baseColor when not explicitly set. */
    const hoverColor = this.options.buttonHoverColor || baseColor;

    // Nothing to apply if neither colour is set
    ...
    /** Switches to hoverColor on mouse enter. */
    ...
    /** Restores baseColor (or CSS defaults) on mouse leave. */
...
/**
 * Tracks how many SocialShareButton instances currently have their modal open.
 * Used to coordinate body scroll-lock across multiple instances on the same page.
 * @type {number}
 */
SocialShareButton.openModalCount = 0;

/**
 * Stores the original value of document.body.style.overflow before the first
 * modal locks it, so it can be restored exactly when all modals close.
 * @type {string|null}
 */
SocialShareButton.originalBodyOverflow = null;
